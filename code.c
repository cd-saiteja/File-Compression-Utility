...
void import_file(FILE *fp_in, unsigned int *freq, const char *file_name)
{
    ...
    // Save the frequency table to a separate table file with the same base name as the input file
...
int main(int argc, char *argv[])
{
    ...
    system("cls");
    printf("*****************************************************************************************\n");
    printf("                                   Data Structures PBL\n");
    printf("                              Akshit | Saiteja | Harshith\n");
    ...
    while (1)
    {
        ...
        case 1:
            ...
            if (argc == 2)
            {
                strcpy(file_name, argv[1]); // Command-line argument directly allows compressing the file
            }
            else if (argc > 2)
            {
            ...
            double compression_ratio = ((double)(input_data - output_data) / input_data) * 100;
